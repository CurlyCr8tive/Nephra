...
import type { KSLSResult } from "@/types/ksls";
      ...
      try {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            // Calculate KSLS based on saved health metrics
            try {
              ...
              if (kslsResponse.ok) {
                ...
                const errorData = await kslsResponse.json();
                console.error("❌ KSLS calculation failed:", errorData);
              }
            } catch (kslsError) {
          ...
          <TabsContent value="health" className="space-y-4">
            <Card>
              <CardContent className="pt-6">
                ...
                <div className="mb-6">
                  <h3 className="font-medium text-sm mb-3">Pulse (Heart Rate)</h3>
                  <div className="flex gap-3">
                    <div className="flex-1">
                      <input 
                        ...
                        max="200"
                      />
                      <span className="text-xs text-neutral-500 block text-center mt-1">bpm (beats per minute)</span>
                      <span className="text-xs text-neutral-400 block text-center mt-0.5">Normal: 60-100 bpm</span>
                    </div>
                  </div>
                </div>
